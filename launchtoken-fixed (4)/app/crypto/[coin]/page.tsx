...
import CryptoComparisonPage from '@/components/crypto-comparison-page';
import CryptoCategoryPage from '@/components/crypto-category-page';

export default function CoinPage() {
  const { coin: coinId } = useParams<{ coin: string }>();

  const CATEGORY_SLUGS = ['next-100x-altcoins','next-100x-ai-coins','next-100x-gaming-coins','next-100x-defi-coins','next-100x-layer1-coins','hidden-gems','biggest-movers'];
  if (CATEGORY_SLUGS.includes(coinId)) return <CryptoCategoryPage />;
